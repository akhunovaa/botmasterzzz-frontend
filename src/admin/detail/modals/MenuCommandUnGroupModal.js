...
import {commandGroupUpdateRequest} from "../../../util/APIUtils";
import Alert from "react-s-alert";

class MenuCommandUnGroupModal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            open: false,
        ...
        if (this.props.selectedRow.id) {
            ...
            return (
                <Modal dimmer="blurring" open={this.props.open} onClose={this.props.onClose} size="tiny"
                    ...
                    <Modal.Header className="modal-header">Разгруппировать пункты меню</Modal.Header>
                        ...
                        <Grid columns={1} textAlign="left">
                            <Grid.Column>
                                <ol className="ol-modal-menu">
                                    ...
                                    <li className="li-modal-menu">
                                        <label className="labelx" id="subMenu" form="childMenu">Удалить подменю</label>
                                        <Dropdown
                                            placeholder='Команда'
                                            fluid
                                            basic
                                    ...
                                    <div className='project-modal-helper-message'>
                                        <label>Для разгруппировки подменю действует ограничение в 5 шт.</label>
                                    </div>
                                </ol>
                            </Grid.Column>
                        </Grid>
    ...
    groupingSave(event) {
        ...
        if (this.state.options.value.length > 5){
            ...
            commandGroupUpdateRequest(commandsGroupSave)
                .then(response => {
                    if (response.error) {
...
export default MenuCommandUnGroupModal